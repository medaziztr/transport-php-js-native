...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$indicatif = mysqli_real_escape_string($db,$_POST['indicatif']);
	
	$passwd = mysqli_real_escape_string($db,$_POST['passwd']); 

	$sql = "SELECT * FROM `transporteur` WHERE (`telephone`='$telephone' AND `password`='$passwd' AND indicatif='$indicatif') OR (`email`='$user' AND `password`='$passwd')";
	...
	if($count == 1) {
		...
		if(isset($_POST['verifcode'])){
			if($row["code"]==$_POST['verifcode']){
				$sql2 = "UPDATE `transporteur` SET `verif`=1 WHERE (`telephone`='$telephone' AND `password`='$passwd' AND indicatif='$indicatif') OR (`email`='$user' AND `password`='$passwd')";
